...
(function() {
    ...
    exportDef = function() {
        ...
        makeClient = function(prefix) {
            ...
            sendPagePerformance = function(path, customMetricsCollectionCallback) {
                var start, time, _ref3, _ref4, _ref5, customMetrics,
                    _this = this;
                ...
                if ((_ref4 = document.readyState) === 'uninitialized' || _ref4 === 'loading') {
                    if (typeof window.addEventListener === "function") {
                        window.addEventListener('load', function() {
                            return setTimeout(function() {
                                return sendPagePerformance.call(_this, path, customMetricsCollectionCallback);
                            }, 500);
                        }, false);
                    }
                    return false;
                ...
                for (key in _ref5) {
                    ...
                    if (typeof time === 'number') {
                        console.log("" + path + "." + key, time - start);
                        timer.send("" + path + "." + key, time - start);
                    }
                }

                if (customMetricsCollectionCallback) {
                    customMetrics = customMetricsCollectionCallback(_ref5, start);
                    for (key in customMetrics) {
                        time = customMetrics[key];
                        if (typeof time === 'number') {
                            console.log("" + path + "." + key, time);
                            timer.send("" + path + "." + key, time);
                        }
                    }
                }
